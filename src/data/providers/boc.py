...
class BOCMacroDataProvider(MacroDataProvider):
    ...
    async def get_interest_rates(self) -> dict:
        ...
        """
        try:
            # No date range — matches the MacroDataProvider ABC (base.py). `recent=1`
            # is enough since only the latest value is used.
            overnight_url = f"{self.BASE_URL}/observations/V39079/json?recent=1"
            overnight_data = await self._fetch_json(overnight_url)

            prime_url = f"{self.BASE_URL}/observations/V80691311/json?recent=1"
