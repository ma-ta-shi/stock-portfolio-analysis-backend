...
Calls go through `/api/chat`, not `/api/generate` — `/api/generate` with
`format: "json"` corrupts gpt-oss:20b output (garbled text or an empty
response despite a real eval_count); `/api/chat` separates the `thinking`
field from `content` before the JSON grammar is applied, so the two don't
collide. See docs/technical/ollama-generate-format-json-finding.md. Whoever
builds the Risk Advisor's Stage A->B `/api/generate` continuation primitive
(86bc2d414) needs that finding, not this one.
"""
...
class BaseRunner:
    ...
    async def call_with_validation(
        ...
        """Call the model with validation and retry on failure. Returns (output, errors).

        Deliberately does NOT catch OllamaUnavailable (raised by the
        preflight check inside call_model on the process's first call only --
        see _preflight_lock above). Both cases it represents -- Ollama isn't
        running, or the model isn't pulled -- are non-transient config
        problems, not network blips: retrying for up to MAX_RETRIES backoff
        cycles cannot fix either one, and doing so would flatten
        OllamaUnavailable's specific, actionable message into a generic
        "Ollama request error" string in `last_errors`, indistinguishable
        from a real timeout. Letting it propagate uncaught and structurally
        distinct is also what will let a future orchestrator tell "the whole
        environment is broken, abort the run" apart from "this one agent had
        a rough call, mark it failed and move on."
        """
        last_result = {}
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                result = await self.call_model(
                    system_prompt, current_user_msg, max_tokens, temperature
                )
                ...
                last_result = result
                last_errors = errors
                current_user_msg = self._build_retry_message(
